...
const selectedEnv =  normalizeEnv(process.env.BUILD_TARGET) || ndeConfig["defaultEnvironment"]; // Is set in build.js
console.log(`\n👉 ng build env: ${selectedEnv}`);
...
const environmentCfg = ndeConfig["environments"][selectedEnv]; 

if (!environmentCfg) {
    console.error(`Error: ❌ Missing environment configuration in package.json for  ${selectedEnv} (e.g. kuleuven, central)`);
    process.exit(1);
}

const build_target = ( selectedEnv === 'central')  ? 'central' :  'view';
const view = ( selectedEnv === 'central')  ? null :  selectedEnv;

const isCentral = build_target === 'central';

const customComponentMappings = isCentral
  ? path.resolve(
      __dirname,
...
class NdeComponentDiscoveryPlugin {
  constructor(options = {}) {
    // central | view
    this.build_target = options.build_target || "central";
  ...
  getScanDirs() {
    ...
    if (this.build_target === "view" && this.view) {
  ...
  generateMappingsContent() {
     console.info(
        `\n[NdeComponentDiscovery] generateMappingsContent for: ${this.build_target} ${this.view}`
      );
...
console.log('[ComponentMappings] Build target:', '${this.build_target}');
