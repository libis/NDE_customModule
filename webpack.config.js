...
class NdeComponentDiscoveryPlugin {
  constructor(options = {}) {
    this.componentsDir = options.componentsDir || path.resolve(__dirname, 'src/app/components');
    this.mappingsFile  = options.mappingsFile  || path.resolve(__dirname, 'src/app/custom1-module/customComponentMappings.ts');
    this.mappingsDir   = path.dirname(this.mappingsFile);
  }

  apply(compiler) {
    compiler.hooks.beforeCompile.tapAsync('NdeComponentDiscoveryPlugin', (params, callback) => {
      try {
        const imports = this.findComponentFiles(this.componentsDir)
          .map(file => {
            let rel = path.relative(this.mappingsDir, file).replace(/\\/g, '/');
...
`;
        ...
        const existing = fs.existsSync(this.mappingsFile) ? fs.readFileSync(this.mappingsFile, 'utf8') : '';
        if (existing !== content) {
          fs.writeFileSync(this.mappingsFile, content);
          console.log(`[NdeComponentDiscovery] Updated mappings with ${imports.length} component(s)`);
        }
      } catch (err) {
...
class NdeInterceptorDiscoveryPlugin {
  constructor(options = {}) {
    this.interceptorsDir = options.interceptorsDir || path.resolve(__dirname, 'src/app/interceptors');
    this.registryFile    = options.registryFile    || path.resolve(__dirname, 'src/app/interceptors/_registry.ts');
    this.registryDir     = path.dirname(this.registryFile);
  }

  apply(compiler) {
    compiler.hooks.beforeCompile.tapAsync('NdeInterceptorDiscoveryPlugin', (params, callback) => {
      try {
        const imports = this.findInterceptorFiles(this.interceptorsDir)
          .map(file => {
            let rel = path.relative(this.registryDir, file).replace(/\\/g, '/');
            if (!rel.startsWith('.')) rel = './' + rel;
            return `import '${rel.replace(/\.ts$/, '')}';`;
          });
...
`;
        ...
        const existing = fs.existsSync(this.registryFile) ? fs.readFileSync(this.registryFile, 'utf8') : '';
        if (existing !== content) {
          fs.writeFileSync(this.registryFile, content);
          console.log(`[NdeInterceptorDiscovery] Updated registry with ${imports.length} interceptor file(s)`);
        }
      } catch (err) {
