...
if (match) {
  ...
  const newBootstrapPath = path.resolve(
    __dirname,
    `src/bootstrap${addonName}.ts`,
  );
...
/*
 ...
 // in deze prebuild die in in app compoenent de import regelt : todo kijjk ook eens naar angular.json
 check if the const subfodler gebruikt kan worden daarvoor 
 // customisatie in code table in plaats van customjs.... json object in codetable pompen, 
 // configuratie cloud app die alles wegschijft naar code table
 wanneer bgeinnen ze te renderen ? naa alles ingelezen of na bootstrap gedaan is, 
 hoe zitt dat met die laoding puntjes en placeholders
